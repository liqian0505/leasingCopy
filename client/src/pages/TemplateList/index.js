                    ...
                    return ( <
                        ...
                        actions = {
                            [ <
                                ...
                                parameters = { item }
                                type = "edit"
                                onClick = { parameters => this.editHandler(parameters) }
                                />, <
                                CustomIcon title = "删除"
                                parameters = { item }
                                type = "delete"
                                onClick = { parameters => this.deleteHandler(parameters) }
                                />, <
                                CustomIcon title = "查看合同列表"
                                parameters = { item }
                                type = "bars"
                                onClick = { parameters => this.filterHandler(parameters) }
                                />, <
                                Icon type = "ellipsis" / >
                            ]
                        } >
                        <
                        CustomInput record = { item }
                        defaultValue = { item.name }
                        onChange = {
                            (id, content) => this.updateHandler(id, content) }
                        /> <
                        /Card> <
                        /List.Item>
                    )
